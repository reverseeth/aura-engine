...
## Input
- `winner_creative_id` — criativo classificado como `breakthrough` pela Skill `ad-analysis` (gate no pre-flight abaixo)
...
## Pre-flight
- [ ] **Gate de rotação — só `breakthrough` rotaciona (cânone `.claude/lib/ad-taxonomy/README.md` §2):** o `winner_creative_id` tem `class == "breakthrough"` em `manifest.ad_classification[]` (gravado pela Skill `ad-analysis`) — equivalente: o id está em `manifest.breakthroughs[]` ou em `ad-analysis/dados.json.breakthroughs[]`. `kpi_winner` NÃO rotaciona: bate o KPI sem puxar spend e o cânone o trata como loser para decisão — gerar variações dele multiplica um resultado de amostra pequena. `spend_winner` também não entra: o destino dele é iteração (Skill `content-recycler`, Movimento 1), não rotação.
- [ ] **Fallback legado** — produto sem `ad_classification[]`/`breakthroughs[]` no manifest (análise antiga ou Skill `ad-analysis` nunca rodada): aplicar o gate antigo (CPA < target × 0.8 E spend > $300 E age > 5 days) e AVISAR o membro antes de prosseguir: "esse produto ainda não tem a classificação canônica da Skill `ad-analysis`; usei o gate legado, que não distingue breakthrough de KPI winner — recomendo rodar `ad analysis` antes de ativar as variações". `manifest.winners[]` legado existe em dois shapes (ids string — alias de `breakthroughs[]` — ou objetos com métricas, em produto antigo); aceite ambos pra LOCALIZAR o criativo, mas presença em `winners[]` sozinha nunca substitui o gate — o array antigo pode carregar `kpi_winner` disfarçado de winner.
- [ ] Skill `creative-engine` disponível pra gerar variations
...
```
winner = read(/workspace/[produto]/creative-engine/concept-XX.md)
dna = read(/workspace/[produto]/creative-dna/dna-profile.json)  # se existe
```

### 2. Gerar N variações preservando DNA (instruções diretas — não existe "modo rotation" na Skill `creative-engine`)

Gere cada briefing de variação diretamente, a partir do conceito-pai:

1. Carregue o concept do winner em `creative-engine/dados.json` (o objeto do concept correspondente — hook, ângulo, mechanism, CTA, proof stack, formato) + o briefing `concept-XX.md` como contexto.
...
O output de cada variação é um briefing novo (`concept-XX-v2.md`, `-v3`, `-v4`) no mesmo formato da Skill `creative-engine` — mas gerado aqui inline, sem re-rodar o fluxo completo da `creative-engine` (a rotação pula ideação/batch: o conceito já está validado, só o eixo varia).

### 3. Rodar DNA extraction em cada
(Automático via ETAPA 7.6 da Skill `creative-engine`)
