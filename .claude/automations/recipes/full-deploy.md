...
Receita orquestradora que faz deploy completo de produto em Shopify + estrutura de campanha no Meta Ads, com um único comando do membro. Estrutura canônica de campanha: **1 campanha com CBO → N ad sets broad/Advantage+ (1 ad set = 1 conceito) → 3 criativos + 2 primary texts + 2 headlines cada** — a MESMA que a Skill `ad-strategy` define e que o cânone `.claude/lib/ad-taxonomy/README.md` §1 governa; esta receita nunca inventa estrutura própria nem recalcula capacidade (quantos ad sets sobem já foi decidido na ETAPA 3.1 da Skill `ad-strategy` e está gravado em `ad-strategy/dados.json`). Tudo nasce PAUSED (inclusive Automated Rules, que nascem DESATIVADAS) até aprovação humana.
...
- [ ] `manifest.json` completo com `ad-strategy` em `skills_completed`
- [ ] `offer-builder/dados.json` existe (`pricing.main_sku_price`, `unit_economics`)
- [ ] `copy-engine/copy-engine.md` pronto (ou o legado `relatorio.md` em produto antigo)
- [ ] `page/staging/` deployed E `manifest.storefront.page_url` preenchido (gravado pela `page-build` — é a URL de destino dos ads)
- [ ] `manifest.tracking.tracking_ready == true` (gravado pela `tracking-setup` — pixel + CAPI ativos, EMQ ≥ 6/10)
- [ ] `creative-engine/dados.json` com o batch de conceitos
- [ ] `ad-strategy/dados.json` com `campaign` (incluindo `budget_level: "campaign_cbo"` e `daily_budget`), `ad_sets[]` (uma entrada por conceito) e `test_budget_daily` definidos
...
result_1 = invoke_recipe("deploy-shopify-product", {
  ...
  bundle_tiers: read("checkout-aov/dados.json").levers.bundles.tiers,  // Solo/3-pack/6-pack; se `checkout-aov` não rodou, single-SKU do `offer-builder` (pricing.main_sku_price)
  description_md: "/workspace/[produto]/copy-engine/copy-engine.md",
...
Ler `strategy = ad-strategy/dados.json`. **Primeiro, checar se a Skill `ad-strategy` ETAPA 6 já criou a estrutura via MCP** (não duplicar):
...
if strategy.mcp_creation.status == "created_paused":
    ...
    // criar agora, com os campos REAIS do schema da `ad-strategy`
    campaign = meta_ads.campaign.create(
      ...
      // Se os dois divergirem, PARAR e reportar: a Skill `ad-strategy` é a fonte do número,
    ...
    for entry in strategy.ad_sets:   // uma entrada por conceito: concept_id, angle, name, daily_max_spending_limit
        ad_set = meta_ads.ad_set.create(
          ...
          // (~3× target CPA, cânone §1), nunca piso. Daily MINIMUM continua proibido (Skill `ad-strategy` ETAPA 7).
          ...
          targeting={
            ...
            // BROAD/Advantage+ — o criativo faz o targeting (Skill `ad-strategy` ETAPA 3).
            // NUNCA hardcodar demografia: age 18-65+, gender All,
            // a menos que a `market-research`/`ad-strategy` tenham dado claro de produto gênero-específico.
        ...
        entry.ad_set_id = ad_set.id   // devolver pro `ad_sets[]` do ad-strategy/dados.json
```
...
for entry in strategy.ad_sets:                  // uma entrada = um conceito
    ...
    for item in rendered where item.file != null:   // item.creative_n = 1..3, na ordem do pack
        invoke_recipe("upload-creative-to-meta", {
          ...
          video_path: "/workspace/[produto]/creative-engine/<item.file>",   // file já vem com "renders/"
          status: "PAUSED"
        })
        entry.ad_ids.append(<ad id retornado>)  // devolver pro `ad_sets[]` do ad-strategy/dados.json
```

Ad Name segue a convenção da `ad-strategy`: `[concept_id]_[creative-n]_[YYYYMMDD]` (ex: `RootCauseAngle_2_20260620`). O `concept_id` preserva o handoff `creative-engine`→`ad-strategy`→`ad-analysis` e o índice `n` (o `creative_n` do item de `rendered_files[]` — 1..3, na ordem das execuções do pack) mantém os 3 ads do conceito distinguíveis no relatório — sem ele, os 3 ads de um mesmo conceito colidiriam no nome. O `utm_content` acompanha, `[concept-id]-[creative-n]`, um por criativo (schema da Skill `ad-strategy` ETAPA 3.3): nunca subir 2 ads com o mesmo `utm_content`.
...
Gravar no bloco `protections` (mesmo shape do `ad-strategy/dados.json`) o que foi efetivamente criado:
...
A receita de sync resolve sozinha o cascade (oficial → Pipeboard → devolve pro manual) e salva o baseline em `/workspace/[produto]/ad-analysis/raw-pull-[timestamp].json` com `source` declarado. Sem MCP Meta nenhum: logar warning — baseline será preenchido manualmente na primeira Skill `ad-analysis`.

### Stage 6 — Update manifest + log

```
manifest.update({
  "10_campaign_id": campaign_id,        // chaves que a Skill `ad-analysis` lê (ad-strategy.md)
  "10_ad_set_ids": ad_set_ids,          // LISTA — um ID por conceito; é por aqui que a `ad-analysis` puxa insights por conceito
...
Devolver também pro `ad-strategy/dados.json` o `ad_set_id` e os `ad_ids` de cada entrada de `ad_sets[]` (preenchidos nos Stages 2 e 3) e o bloco `protections` do Stage 4 — é o estado que a Skill `ad-analysis` lê pra saber o que existe de verdade na conta.

Se a estrutura veio da Skill `ad-strategy` (`mcp_creation`), os IDs já estavam no manifest — confirmar, não sobrescrever com null nem truncar a lista `10_ad_set_ids` para um único ID.

**Ad log (cânone `.claude/lib/ad-log/README.md`) — na MESMA execução:** append em `workspace/[produto]/ad-log.md` (criar com o cabeçalho da tabela se não existir), uma linha por mudança que ESTE deploy executou na conta — a campanha, cada ad set, as duas automated rules. Os ads são registrados pela sub-receita de upload no Stage 3 (não duplicar aqui). O que a Skill `ad-strategy` já criou antes (`mcp_creation`) também não entra — ela mesma logou na hora. Modo `dry-run` não escreve nada (nenhuma mudança real aconteceu).
...
Meta Ads (tudo PAUSED):
  ...
  · Kill e escala automáticos por performance não existem aqui — quem lê e decide é a
    "ad analysis" (Skill `ad-analysis`), depois de 3 dias de dados

Creative DNA Registry (estado herdado — o registro aconteceu na Skill `creative-engine` ETAPA 7.6, não neste deploy):
  · <N> criativos registrados pela Skill `creative-engine` ETAPA 7.6, com features extraídas
  ⏳ Performance update virá via Skill `ad-analysis` após 3 dias
...
- Stage 2 (Meta setup — campanha + N ad sets): 30-60s (ou 0s, se a Skill `ad-strategy` já criou)
- Stage 3 (upload vídeos — só os existentes): 20-40s por vídeo
- Stage 4 (proteções): 10-20s
- Stage 5 (sync): 20-30s
- **Total: 2-5 minutos** pra campanha típica de N ad sets × 3 ads (N = `adsets_planned` da Skill `ad-strategy`)
