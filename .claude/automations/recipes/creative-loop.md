...
|---|---|---|
| **Nunca publish autônomo** | Toda variação nova sobe em `PAUSED`; pause de ad/ad set só com confirmação do membro. O loop NUNCA ativa nada. | doutrina Skill `ad-strategy` ETAPA 6 |
| **Janela de teste intocável** | Campanha com < 72h de dados → o loop SÓ observa (nenhuma recomendação de mexer). Mexer cedo destrói o sinal. | Skill `ad-strategy` ETAPA 5 |
| **Spend cap** | Ler `loop_spend_cap_daily` do manifest (default = `test_budget_daily` da `ad-strategy`). O cap vale sobre o budget da **CAMPANHA** — é lá que o CBO vive; ad set de teste não tem budget próprio, só o `daily maximum` de proteção. O loop nunca recomenda budget total acima do cap; subir o cap é decisão explícita do membro, fora do loop. | membro |
| **Piso de ROAS** | Se ROAS da campanha < `breakeven_roas` (`offer-builder` `unit_economics`), ZERO recomendação de escala — só refresh criativo + apontar diagnóstico da Skill `ad-analysis`. | Skill `offer-builder`/`scale-engine` |
| **Incrementos < 20%** | Ajuste de budget recomendado por ciclo ≤ +20% (default +5%, estilo "farmer" da Skill `scale-engine`) — incremento maior reseta a learning phase. | Skill `scale-engine` |
| **Estrutura respeitada** | 1 campanha com CBO → N ad sets (1 ad set = 1 conceito) → 3 criativos cada. O loop troca CRIATIVOS **dentro do ad set do conceito correspondente**; nunca cria campanha nova nem empilha conceitos diferentes no mesmo ad set. Ad set novo existe só pra conceito NOVO, dentro do teto de capacidade (Step 5). Diversificação em campanha ABO paralela continua sendo Skill `scale-engine`, pós-breakthrough. | contrato da Skill `ad-strategy` + cânone `.claude/lib/ad-taxonomy/README.md` §1/§5 |
...
## Pre-flight
- [ ] `manifest.10_campaign_id` existe (campanha criada pela `ad-strategy` / full-deploy) e campanha ATIVA
...
- [ ] `offer-builder/dados.json` (`unit_economics.breakeven_roas`, target CPA) + `ad-strategy/dados.json` disponíveis — deste último, `ad_sets[]` (o mapa conceito → ad set: `concept_id`, `name`, `ad_set_id`, `ad_ids`, `daily_max_spending_limit`) e `test_capacity` (`max_adsets`, `adsets_planned`, `binding_constraint`)
...
O `ad_class` do pull é **pré-classificação, não veredito**: quem manda é o recálculo da Skill `ad-analysis` no Step 2, com a análise completa. Ad `unclassified` é o caso normal de teste jovem, nunca sinal negativo.

### Step 2 — Leitura (doutrina da Skill `ad-analysis`, versão resumida)
...
- **Checks de precedência:** antes de culpar criativo, conferir funil (ATC→purchase), CPM da conta e `dataset_health` (EMQ < 6/10 infla CPA por undercounting). Problema estrutural → recomendar Skill `ad-analysis` completa, não mexer em criativo.

> Ambiguidade em qualquer leitura (dados contraditórios, spend baixo demais pra concluir) → parar o loop e recomendar a Skill `ad-analysis` completa. O loop é o ritual rápido; a `ad-analysis` é o diagnóstico profundo.

### Step 3 — GATE HUMANO 1: digest de decisões

Apresentar ao membro um digest curto (não relatório — o relatório é da Skill `ad-analysis`):
...
- Pausas aprovadas → pause de **ad individual**: `mcp__meta__ads_update_entity(entity_id=ad_id, entity_type="ad", status="PAUSED")` (ou o equivalente Pipeboard). O loop pausa o AD, nunca o ad set: pausar o ad set inteiro é matar o CONCEITO, decisão da régua de conta madura do cânone §3 e da Skill `ad-analysis` — `pause-ad-set.md` é receita de emergência (ad set/conta), não deste loop.
...
### Step 4 — Gerar variações dos breakthroughs (Skill `creative-engine` + Higgsfield)
...
- A rotação usa o Creative DNA Registry (`creative-dna/`) pra preservar o que funcionou e o gate de diversidade da Skill `creative-engine` pra não subir 3 clones (criativos quase iguais contam como ~1 pro sistema de entrega da Meta).
- **Com Higgsfield MCP:** renderizar in-session (confirmar créditos com o membro antes — ETAPA 0.7 da Skill `creative-engine`); salvar em `creative-engine/renders/`.
...
**Conceito NOVO** (não é variação de nenhum conceito no ar — ex: batch novo da `creative-engine` entrando por fadiga): ele precisa do **próprio ad set** (cânone §1: 1 ad set = 1 conceito; empilhar no ad set de outro conceito ainda gasta, mas mata a leitura de qual conceito funcionou). Antes de criar, checar a capacidade que a Skill `ad-strategy` já gravou em `test_capacity`:
...
- **Não cabe:** o conceito novo entra na FILA, ou substitui o ad set do conceito mais fraco (o membro escolhe qual sai). **Nunca** subir budget pra abrir vaga dentro do loop: mudar a capacidade é mudar o teste, e isso é decisão explícita do membro fora do loop (Skill `ad-strategy` ETAPA 3.1).
...
- Não cria campanhas novas; ad set só cria quando um conceito NOVO entra e a capacidade da `ad-strategy` comporta (Step 5). Escala estrutural — o ABO paralelo do breakthrough (cânone §5) — continua sendo Skill `scale-engine`
- Não substitui a Skill `ad-analysis` (o digest é a leitura rápida; diagnóstico profundo de losers, funil e conta é a `ad-analysis`)
- Não roda sem MCP Meta (sem dados confiáveis não há loop — cair pra Skill `ad-analysis` manual)
