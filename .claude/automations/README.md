...
|---|---|---|
| **Meta MCP oficial** (preferencial) | `https://mcp.facebook.com/ads` | Open beta desde 2026-04-29, rollout gradual, 29 tools |
...
- `sync-campaign-from-meta-official.md` — **preferencial.** Puxa estado completo via MCP oficial da Meta. Inclui industry benchmarks, dataset quality, auction ranking, anomaly signal.
- `sync-campaign-from-meta.md` — fallback automático via Pipeboard quando o oficial está disabled no ad account ou indisponível.
- `upload-creative-to-meta.md` — sobe vídeo aprovado pro Meta Ads Manager com UTM + pixel wired, modo pausado (humano ativa). Continua via Pipeboard ou Playwright porque o MCP oficial não aceita arquivo local.
- `pause-ad-set.md` — pausa ad set por CPA/freq threshold. Cascade oficial → Pipeboard.
- `deploy-shopify-product.md` — cria produto + 3 variants (Starter/Popular/BestValue) + wire Variant IDs no template da PDP
- `rotate-winning-creative.md` — detecta winner, sobe variações no Meta
- `full-deploy.md` — orchestra full launch (Shopify product + Meta ads paused)
