...
Puxe o framework nomeado: **Hero Sections (5 Types + Selection)** (rode `hero sections 5 types value prop dreamstate problem segment campaign selection`) pra ter os 5 tipos canônicos (value-prop, dreamstate, problem, segment, campaign) e os critérios de seleção. Cruze com **StoryBrand 5 Website Essentials** (rode `StoryBrand five website essentials above the fold offer CTA images of success`) e valide a clareza do topo com o **Grunt Test** (rode `StoryBrand grunt test 5 second clarity hero section three questions`) — o herói tem que passar nos 3 segundos. Escolha 1 hero baseado no `page_type`, awareness, e na natureza do produto (transformação visual vs commodity vs categoria nova). Registre o `hero_type` no strategy block — a rota de design da ETAPA 3 e a `page-build` usam isso pra dar a direção do hero. Pra advertorial, o "hero" tende a ser editorial (headline + dek + abertura de história); pra landing/pdp, benefit-forward com CTA e proof acima da dobra.
