...
Aqui sai só direção de cor, tipografia e densidade, nunca o layout. Leia `brand.md` primeiro e pergunte em uma mensagem só o que faltar. Três sub-etapas:

- **2.1 Tipografia** (`reference/tipografia.md`): abre com duas famílias nomeadas e só duas, lidas do `suggested_typefaces` de `.claude/lib/design-presets/presets.json` — ABC Oracle, que o membro baixa na fundição e guarda em `workspace/fontes/`, e Geist, que carrega por link. A Aura não baixa nem hospeda fonte, e nada sobre licença entra no que o membro lê. Arquivo local passa SEMPRE pelo `.claude/lib/design-presets/local_fonts.py` (`scan` pra detectar, `copy` pra levar os pesos usados pra `design/assets/fonts/`, `css --mode relative` pro bloco `data-aura-fonts`, que é o que a `page-build` troca pelo asset do tema). Tipografia do `brand.md` vence tudo; recusou as duas, a família vem da 2.2. Grave o bloco `typography`.
- **2.2 Cor** (`reference/brand-signals.md`): Caminho 1, Refero MCP (`mcp__refero__`, 2 a 3 candidatos, o membro escolhe); Caminho 2, screenshot full-page lido por visão (fallback primário); Caminho 3, `tools/design-clone/aura_clone.py` só pra hex exato (ignore `defaults_fallback`); Caminho 4, o gerador `.claude/lib/design-presets/palette_engine.py` (3 candidatas com motivo, harmonia declarada e AA garantido; os 8 presets são a base dele, nunca um menu). A família decidida na 2.1 vence a que qualquer caminho trouxer.
- **2.3 Comparadora** (`reference/brand-signals.md`): obrigatória sempre que sobrar mais de uma candidata em cor ou em tipografia, com um seletor por eixo, nos trios R,G,B do `--format css`; dois temas pedem um snippet de paleta por tema, com a disciplina de push da rule `shopify-theme-safety.md`. Salve `design-signals.json` e mostre o resumo ao membro.
