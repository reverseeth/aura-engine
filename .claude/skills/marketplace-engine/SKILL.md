...
4. Base pelo índice (domínio `affiliate-creator-channels`, mais entradas de canal em `meta-ads-strategy`, `scaling` e `creatives-hooks-formats`): `python3 .claude/lib/kb-index/kb_lookup.py --skill marketplace-engine --domain <domínio desta etapa>`; `best_query` exata com `deep=true`, no máximo 6 buscas por etapa; a lista completa, com as queries exatas e o recorte do que fica com as vizinhas, está em `reference/sistemas.md`; nunca query genérica nem busca repetida.
