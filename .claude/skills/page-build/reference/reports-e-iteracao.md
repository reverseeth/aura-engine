...
- **`page_type: quiz`: re-compilar a section do funil apaga tudo de uma vez** (os três block types renomeados, os `variant_id` das telas de resultado e as `key` de destino preenchidas no template). Depois de qualquer re-COMPILE dela, refaça o rename, o wire dos IDs e confira as quatro peças do CSS na ordem de `reference/quiz-sections.md` antes do push. Mudar só o texto de uma tela é edição de setting no template JSON, sem recompilação nenhuma.
