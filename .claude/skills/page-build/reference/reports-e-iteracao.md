...
> O conteúdo do `page-report.md`, o schema do `deploy-report.json`, a atualização do manifest, o iteration loop por tipo de ajuste (markup, estrutural, re-compile, preço, FAQ), a mensagem final e o bloco de self-audit expandido desta skill. Abra na ETAPA 7 e antes de declarar concluído.
...
- **Preço/rating/política mudou** → regenere o `product-schema.json` e o bloco agent-facts (ETAPA 4.5), revalide o JSON-LD, e confira que ainda bate com a config Shopify (ETAPA 4.5.1) antes do re-push. Schema e config nunca podem divergir.
