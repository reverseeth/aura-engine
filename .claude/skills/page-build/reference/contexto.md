...
8. DEPLOY — shopify-theme-safety integral (detecção da loja → **produto e oferta de pé na loja, com os IDs de variante ligados a todas as superfícies de compra e o check bloqueante de IDs** → duplicate → pull --nodelete → **provisionamento de web fonts** → push --nodelete + marker `data-aura-build` + criação da página + smoke test).
...
> A oferta da `offer-builder` só existe na loja porque esta skill a coloca lá (6.1b): produto com uma variante por quantidade, pacote, assinatura, desconto automático e cupom pelo caminho nativo do Shopify, e os IDs gravados em `manifest.storefront` pra `checkout-aov` e `bonus-delivery` lerem. Detalhe em `reference/produto-e-oferta.md`.
