...
- Se `launch_recommendation == "BLOCK"` → **ABORTAR**. Os fluxos de email vão herdar o drift detectado (mecanismo divergente, VOC sem rastreio, oferta diferente da página) e propagar inconsistência pra base de subscribers. Pedir correção via skill 09 primeiro.
...
2. Membro escolhe
3. Skill gera HTML de cada email adaptando ao produto (usa `06-copy.md` como base de copy + `02-market-research.md` pra VOC + `04-offer.md` pra mecanismo)
...
2. `/workspace/[produto]/13-retention/[fluxo]/setup-guide.md` com step-by-step manual no dashboard do ESP
