...
- Python 3: `python3 --version` — necessário pra pipeline de design-clone (skill `page-design` — rota de clone com o SingleFile e brand signals). Mac já vem com Python 3.
- **single-file-cli (opcional — snapshot de página, via npm):** usado por `tools/design-clone/snapshot.py` (rota de clone com o SingleFile da skill `page-design`) pra capturar uma página de referência inteira num único arquivo HTML. Não precisa instalar globalmente — roda on-demand via npx. Teste rápido: `npx -y -p single-file-cli single-file --help >/dev/null 2>&1 && echo "disponível"`. Se npm/npx estiver ausente ou o download falhar, nada quebra: a `page-design` segue pelo caminho principal da rota, que é o arquivo salvo pelo membro com a extensão SingleFile do Chrome.
...
- **Venv do design-clone (RECOMENDADO — rota de clone com o SingleFile e brand signals da `page-design`):** os scripts de `tools/design-clone/` (downloader, analyzer, pattern-extractor, liquid-converter) precisam de Playwright/bs4 num venv PRÓPRIO (mesma razão PEP 668). Crie logo após o venv do fetcher:
  ```bash
