...
**Passada 1b — outra ordenação, quando a 1 devolver pouca coisa.** Repita a mesma passada trocando só a ordenação, para `sort_by: "adOrder"` com `order: "asc"` (valor menor é rank mais forte), mantendo o `created_after`. Ela ordena por força de rank em vez de tempo no ar, e às vezes sobe criativo que a primeira deixou embaixo. O recorte de vencedor recente continua o mesmo.
...
**As armadilhas da ferramenta, as quatro obrigatórias:**

1. **`sort_by` sempre definido, e nunca o `trend_signal`.** São dois parâmetros diferentes. Omitir o `sort_by` cai no `trend_signal` padrão, o de crescimento de sete dias, que injeta por conta própria janela de alcance recente e alcance mínimo de 1 — dois filtros que ninguém pediu e que descartam anúncio sem dado de alcance. O guard de alcance zero pertence ao `trend_signal: "longest_running"`; o `sort_by: "longestRunning"` não tem guard nenhum e ordena por dias no ar, que é dado público de todo anúncio. Medido em 18/09/2026: com `ad_countries: {include: ["US"]}` e `sort_by: "longestRunning"`, a busca devolveu 788 mil anúncios e linhas com 1.645 a 1.831 dias no ar, todas com os EUA no alvo.
2. **Para recortar mercado, use `ad_countries`.** Ele filtra a lista de países-alvo, que existe em todo anúncio. O `main_countries` vem da transparência da União Europeia e da Grã-Bretanha e não existe para os EUA: filtrar por ele não devolve nada de lá. O campo `mainCountry` de cada linha tem a mesma limitação e vem nulo fora daquele escopo, então ele não serve para saber onde o anúncio roda — quem responde isso é `targetedCountries`.
3. **Confirme linha por linha** que o tipo de mídia é vídeo e que os dias no ar batem com o filtro pedido; linha que não bate sai da lista, em vez de confiar na ordenação.
4. **Alcance não é régua de escala aqui.** Alcance e gasto estimado derivam da transparência da União Europeia, então anúncio que roda só nos EUA vem com alcance baixo ou ausente, e isso não é gasto baixo. A escala se lê por dias no ar, duplicatas e rank do anúncio, a mesma régua da ETAPA 3F.
