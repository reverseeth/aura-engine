# Competitor Analysis · Referência: Síntese estratégica e fontes (ETAPA 7)

> Os nove sistemas a puxar e os sete blocos do relatório (mapa competitivo, padrões do mercado, oportunidades ranqueadas, recomendação de posicionamento, swipe file com como adaptar, validated library, resumo e conclusão), mais a regra das fontes só no `dados.json`. Abra na ETAPA 7.
...
**6. Validated Library (ativo estruturado pro `dados.json` — alimenta a Rota A da Skill `offer-builder`)**
...
A Skill `offer-builder` (ETAPA 2A — Rota A) usa esse bloco pra APRIMORAR um mecanismo validado (mantendo o ângulo) ou CRUZAR mecanismo validado × ângulo validado de outra marca/vertical. Sem validated_library preenchida, a `offer-builder` cai na Rota B (criação original) — mais cara e arriscada. Preencha mesmo com poucos itens: 2 mecanismos com evidência real valem mais que 10 sem.
