...
> **Contrato de cobertura (kb-index, revisado 2026-09):** toda decisão de CONTEÚDO/design nasce na 07a — esta skill não re-decide página. Mas nos pontos de implementação MANUAL onde ela decide sozinha (selling plan/Subscribe & Save, buy box/pricing table, desconto Shopify, countdown — a lista "O que o conversor NÃO faz"), a base entra: abra `.claude/lib/kb-index/frameworks.json`, **enumere as entradas cujo `use_in_skill` inclui a 07b** e rode a `best_query` EXATA de cada uma relevante ao ponto (`deep=true`) — NUNCA query genérica. As queries que esta skill cita são núcleo mínimo, nunca o teto; framework já puxado na sessão não se puxa de novo. O tamanho real do domínio é o do `frameworks.json`, nunca número decorado em texto de skill.
...
- **Subscribe & Save / selling plan — não gerado pelo conversor; implementação manual COM os números da 04 (contrato 04→07b).** Antes de montar qualquer opção de assinatura na PDP, leia `04-offer-builder/dados.json` → `subscription_architecture` + `onetime_premium_pct` + `pricing.main_sku_price` e renderize DE ACORDO:
  - `subscription_first` → a PDP tem selling plan com DUAS opções e os preços derivados da oferta: **assinatura = `main_sku_price` (o preço-base)**; **one-time = `main_sku_price × (1 + onetime_premium_pct/100)`** (~15% acima no default da 04 — arredondamento segue a convenção de charm pricing que a 04 já validou contra a âncora de concorrente; na dúvida de arredondamento, o preço final é o do `offer-builder.md`, não um recálculo seu). O framing é o prêmio invertido ("you pay more by NOT subscribing") — **NUNCA escreva "Subscribe & Save X%"**: o desconto de assinatura foi abolido pela 04 (`sub_discount_pct` sai sempre `0`; o número vivo é `onetime_premium_pct`). O markup continua manual: `<input type="radio" name="selling_plan" value="ID">` do app de subscription, com o ID real do selling plan.
