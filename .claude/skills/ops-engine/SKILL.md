...
4. Base pelo índice (domínio `ops-scale-risk`, mais três entradas reaproveitadas de outros domínios): `python3 .claude/lib/kb-index/kb_lookup.py --skill ops-engine --domain <domínio desta etapa>`; `best_query` exata com `deep=true`, no máximo 6 buscas por etapa; a lista completa, com as queries exatas, está em `reference/sistemas.md`; nunca query genérica nem busca repetida.
