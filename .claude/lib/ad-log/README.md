...
- **executor**: `skill-ad-strategy` · `skill-scale-engine` · `recipe:full-deploy` · `recipe:creative-loop` · `recipe:pause-ad-set` · `membro` (quando ele relata mudança manual)
...
- **Skill `ad-strategy`** — criação de campanha/ad sets/ads (mesmo em PAUSED) e automações de proteção.
- **Skill `scale-engine`** — todo ajuste de budget (subida, descida, reset da meia-noite), duplicação em ABO, graduação pra ASC.
- **Skill `content-recycler`** — é planejadora: entrega o plano de amplificação e NÃO executa na conta; a execução (e a linha no log) fica com a Skill `scale-engine` e as receitas. Se algum passo do plano for executado na conta ainda na sessão da `content-recycler`, quem executa loga.
- **Skill `promo-engine`** — tudo que a janela promocional muda na conta: campanha promo, budget direto da promo, surf/recuo intradiário, resets da janela, rule de fim de promo, religada do evergreen.
...
- **Skill `ad-analysis`** — SEMPRE, no início da análise: cruzar a janela de leitura com as mudanças do período ("a queda de quinta coincide com o quê?"). Mudança sem efeito esperado e efeito sem mudança conhecida são os dois achados que só este log revela.
- **Skill `scale-engine`** — antes de escalar: a última mudança de budget e há quanto tempo (o gate de 24h entre degraus é verificado aqui, não de memória).
