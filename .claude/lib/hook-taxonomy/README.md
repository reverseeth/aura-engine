...
Catálogo de archetypes de hook pra ad creatives. Usado pela Skill `creative-engine` pra gerar variações com cobertura de emotions e formatos diferentes, e pela Skill `competitor-analysis` pra classificar criativos escalados dos concorrentes.
...
2. **Archetype NÃO substitui a VOC** — hook que nasce de uma frase real de cliente registra o `ref_id` dela no `market-research/dados.json` (é o que deixa a Skill `ad-analysis` medir qual frase vendeu).
...
- **Skill `competitor-analysis` Etapa 3C** (scaled creative deep analysis): classifica criativos escalados dos concorrentes usando esse taxonomy. Preenche `hook_archetypes[]` em `competitor-analysis/creative-patterns.json`.
- **Skill `creative-engine` Etapa 4.5.E + Etapa 7**: cada hook gerado declara archetype + emotion. Hooks Bank final tem cobertura balanceada. Na ETAPA 7.6, o archetype declarado entra no registry via a feature `hook_archetype` do creative DNA (`feature_schema.json`).
- **Skill `ad-analysis`** (ad-analysis): ao marcar winners/losers, a performance conecta-se ao `hook_archetype` já registrado — o padrão por archetype emerge no `dna-profile.json` ao longo do tempo.
