...
3. **Se barrado (403/429/Cloudflare/login-soft/conteúdo só-JS) → o fetcher Playwright da Aura:**
   ...
   - **Google Trends** → sempre `--mode trends` (aceita o termo direto ou a URL do explore; defaults US/5 anos). NUNCA tentar renderizar o site do Trends com `--mode text` — ele recusa navegador automatizado. O modo consulta a API de dados JSON do próprio Trends (HTTP puro → mesma API por dentro do Chromium) e devolve a série + classificação QUEDA/FLAT/SUBINDO/SPIKE pronta pra ETAPA 2 do product research. Pra muitos termos (uma rodada de product research mede 30-40), use `python3 .claude/lib/web-fetch/trends_batch.py --out <pasta> "termo 1" "termo 2" ...` em segundo plano: o endpoint bloqueia por 30-60 min depois de ~25-30 consultas seguidas, e o lote espaça, pausa, tenta de novo e retoma. Degrau final se um termo continuar bloqueado: screenshot do membro.
