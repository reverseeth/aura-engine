...
| **design-presets** | Fonte única dos 8 presets de design da `page-design` (`presets.json` com os tokens completos e fixos). Existe pra que o mesmo preset devolva sempre os mesmos tokens — a skill lê o arquivo, nunca gera de cabeça. Caminho 4 (último) da cascade de brand signals. | `page-design`, `page-build` |
...
**Cascade de brand signals na skill page-design ETAPA 2:** Refero → screenshot→visão (membro tira print full-page da loja de referência e o Claude lê a imagem com visão nativa — imune a Cloudflare/JS/markup bagunçado; fallback primário de inspiração) → `tools/design-clone/` (Playwright, caminho 3 opcional pra hex exato) → manual / 8 presets.
