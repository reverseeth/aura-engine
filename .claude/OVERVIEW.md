...
**Banco de marcas:** no **Notion** (página-mãe + banco de dados com uma página por marca + página de ranking) se o MCP estiver conectado; senão `banco-de-marcas.html` na pasta do produto. O `.md` é salvo sempre (02/03 leem).
**Output:** `01-product-research/product-research.md` + `.html` + `dados.json` + `banco-de-marcas.md` (+ `.html` sem Notion) + `brand.md` do vencedor
...
**Output:** `06-copy-engine/copy-engine.md` + `copy-engine.html` + `dados.json`
...
Cross-phase drift detection: mecanismo, awareness stage, VOC, oferta concordam entre todos os artefatos (skills 02 → 03 → 04 → 06 → 07 → 08). Cobertura de VOC, garantia/bônus/urgência iguais entre oferta, página, checkout e ads. Roda ANTES do launch, conferindo a página já no ar + criativos + oferta.
**Vira gate de launch:** em `BLOCK`, a 10 aborta (só prossegue com pedido explícito do membro); a 13 bloqueia por default mas oferece prosseguir com `skipped_preflight` reconhecido (filosofia never-stuck). A página existir (07b) não gasta dinheiro; os ads (10) sim — por isso o gate fica antes da 10, não do deploy da página.
...
**Output:** `14-content-recycler/content-recycler.md` + `content-recycler.html` (índice) + `[source-id]/` com `essence.json`, `amplification-plan.md/.html`, `creator-report.md/.html` e — se a Trilha 2 rodou — 9 `.md` + 9 `.html`
...
| **mcp-detect** | Fonte única de verdade dos prefixos de tool MCP detectáveis (`mcp__trendtrack__`, `mcp__meta__ads_*`, `mcp__meta-ads__`, `mcp__refero__`, `mcp__klaviyo__`, `mcp__shopify_dev__`, `mcp__stripe__`, `mcp__claude_ai_Notion__`/`mcp__notion__`) + convenção de log `source`. | 01, 07c, 10, 11, 13 + receitas de automação |
...
| **trendtrack-integration** (opcional) | Integração read-only com MCP TrendTrack (30+ tools; `search_ads` com os filtros do Explorer é o motor de descoberta da 01). Detecção runtime: tools com prefixo `mcp__trendtrack__`; checa créditos antes de gastar. Sem MCP, a 01 explica a pesquisa manual no browser. | 01, 03, 08, 11, 13 |
...
├── 01-product-research/   → product-research.md / .html + dados.json + banco-de-marcas.md (+ .html sem Notion)
...
├── 06-copy-engine/        → copy-engine.md / .html + dados.json
